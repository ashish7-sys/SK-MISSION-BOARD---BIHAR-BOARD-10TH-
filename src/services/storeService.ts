import { PDFMaterial, YouTubeVideo, Announcement, AppVersionInfo, Chapter, SubjectId, MusicTrack, RemoteFeatureFlags, AppBranding, AppHealthScanResult, DiagnosticIssue } from "../types";
...
export const DEFAULT_BRANDING: AppBranding = {
  ...
  appSubtitle: "Class 10 BSEB 2026",
  logoUrl: "",
  themeVideoUrl: "/bg_theme.mp4",
  accentTheme: "cosmic"
};
...
let cachedBranding: AppBranding = { ...DEFAULT_BRANDING };
...
try {
  ...
  const localBranding = localStorage.getItem(STORAGE_KEYS.BRANDING);
  if (localBranding) {
    const parsed = JSON.parse(localBranding);
    cachedBranding = { ...DEFAULT_BRANDING, ...parsed };
  }
...
export const StoreService = {
  ...
  // Branding (App Title, Subtitle, Logo, Accent) API
  getBranding: (): AppBranding => {
    return { ...cachedBranding };
  },

  saveBranding: async (brandingUpdate: Partial<AppBranding>) => {
    cachedBranding = { ...cachedBranding, ...brandingUpdate };
    localStorage.setItem(STORAGE_KEYS.BRANDING, JSON.stringify(cachedBranding));
    if (brandingUpdate.themeVideoUrl) {
      cachedThemeVideo = brandingUpdate.themeVideoUrl.trim();
    ...
    if (db) {
      try {
        await setDoc(doc(db, "meta", "branding"), {
          ...cachedBranding,
          updatedAt: new Date().toISOString()
        }, { merge: true });
      } catch (e) {
        console.warn("Firestore save branding error:", e);
      }
    }
  },
  ...
  getThemeVideoUrl: (): string => {
    return cachedBranding.themeVideoUrl || cachedThemeVideo;
  },

  setThemeVideoUrl: async (url: string) => {
    cachedThemeVideo = url.trim();
    cachedBranding.themeVideoUrl = url.trim();
    localStorage.setItem(STORAGE_KEYS.THEME_VIDEO, cachedThemeVideo);
    localStorage.setItem(STORAGE_KEYS.BRANDING, JSON.stringify(cachedBranding));
    notifySubscribers();
    ...
    if (db) {
      try {
        ...
        await setDoc(doc(db, "meta", "branding"), { themeVideoUrl: cachedThemeVideo, updatedAt: new Date().toISOString() }, { merge: true });
      } catch (e) {
  ...
  initRealtimeSync: async () => {
    ...
    try {
      ...
      // 8. Remote Branding Listener (Logo, App Title, Subtitle, Background)
      const brandingDoc = doc(db, "meta", "branding");
      onSnapshot(brandingDoc, (snap) => {
        if (snap.exists()) {
          const data = snap.data() as Partial<AppBranding>;
          cachedBranding = { ...DEFAULT_BRANDING, ...data };
          if (data.themeVideoUrl) {
            cachedThemeVideo = data.themeVideoUrl;
            localStorage.setItem(STORAGE_KEYS.THEME_VIDEO, data.themeVideoUrl);
          }
          localStorage.setItem(STORAGE_KEYS.BRANDING, JSON.stringify(cachedBranding));
          notifySubscribers();
        }
      }, (err) => {
        console.warn("Firestore Branding sync warning:", err);
      });
  ...
  saveFeatureFlags: async (flags: Partial<RemoteFeatureFlags> | RemoteFeatureFlags) => {
    cachedFeatureFlags = { ...DEFAULT_FEATURE_FLAGS, ...cachedFeatureFlags, ...flags };
    localStorage.setItem(STORAGE_KEYS.FEATURE_FLAGS, JSON.stringify(cachedFeatureFlags));
    notifySubscribers();
  ...
  // Automated Diagnostics & Self-Healing App Scan
  runHealthScan: (): AppHealthScanResult => {
    const issues: DiagnosticIssue[] = [];
    let score = 100;

    // 1. Check Feature Flags Integrity
    const disabledFlags = Object.entries(cachedFeatureFlags).filter(([_, val]) => val === false);
    if (disabledFlags.length > 0) {
      const names = disabledFlags.map(([k]) => k).join(", ");
      issues.push({
        id: "flag-disabled-warning",
        title: `${disabledFlags.length} फ़ीचर फ्लैग्स निष्क्रिय (Disabled) हैं`,
        description: `वर्तमान में [${names}] फ़ीचर्स विद्यार्थियों के लिए बंद हैं।`,
        severity: "warning",
        category: "flags",
        suggestedAction: "सभी आवश्यक फ़ीचर फ्लैग्स को तुरंत सक्रिय (Enable) करें।",
        autoFixPayload: {
      ...
      score -= Math.min(25, disabledFlags.length * 6);
    }

    // 2. Check PDFs Content & Links
    const pdfsWithMissingUrl = cachedPdfs.filter(p => !p.fileUrl || p.fileUrl.trim() === "");
    if (pdfsWithMissingUrl.length > 0) {
      issues.push({
        id: "pdf-missing-url",
        title: `${pdfsWithMissingUrl.length} PDFs में फाइल लिंक अनुपस्थित हैं`,
        description: `कुछ स्टडी मटेरियल PDFs में URL खाली या अमान्य है।`,
        severity: "critical",
        category: "resources",
        suggestedAction: "डिफ़ॉल्ट सत्यापित PDF लिंक्स रीस्टोर करें या लिंक अपडेट करें।",
        autoFixPayload: {
    ...
    // 3. Check Videos Content
    const videosWithMissingId = cachedVideos.filter(v => !v.youtubeVideoId || v.youtubeVideoId.trim() === "");
    if (videosWithMissingId.length > 0) {
      issues.push({
        id: "video-missing-id",
        title: `${videosWithMissingId.length} वीडियो लेक्चर्स में YouTube ID अमान्य है`,
        description: `वीडियो प्लेयर लोड होने में त्रुटि आ सकती है।`,
        severity: "warning",
        category: "resources",
        suggestedAction: "आधिकारिक BSEB Class 10th वीडियो लेक्चर्स सिंक करें।"
      });
      score -= 15;
    }

    // 4. Check App Branding & Theme
    if (!cachedBranding.appName || cachedBranding.appName.trim() === "") {
      issues.push({
        id: "branding-empty-name",
        title: "ऐप का नाम / शीर्षक खाली है",
        description: "हेडर में डिफ़ॉल्ट ब्रांडिंग टाइटल लोड नहीं हो रहा है।",
        severity: "warning",
        category: "branding",
        suggestedAction: "डिफ़ॉल्ट नाम 'SK MISSION BOARD' रीस्टोर करें।",
        autoFixPayload: {
    ...
    const activeAnn = cachedAnnouncements.filter(a => a.isActive !== false);
    if (activeAnn.length === 0) {
      issues.push({
        id: "no-active-announcement",
        title: "कोई सक्रिय परीक्षा सूचना/घोषणा नहीं है",
        description: "होम स्क्रीन पर विद्यार्थियों के लिए लेटेस्ट बोर्ड एग्जाम नोटिस सक्रिय नहीं है।",
        severity: "info",
        category: "resources",
        suggestedAction: "मैट्रिक परीक्षा 2026 हेतु नई गाइडलाइन घोषणा प्रकाशित करें।",
        autoFixPayload: {
          actionType: "CREATE_ANNOUNCEMENT",
          payload: {
            announcement: {
              title: "मैट्रिक परीक्षा 2026 महत्वपूर्ण सूचना",
              message: "सभी छात्र-छात्राएं अपनी परीक्षा की तैयारी हेतु मॉडल पेपर्स और वीडियो लेक्चर्स अवश्य देखें!",
              type: "IMPORTANT",
              isImportant: true
            }
          }
    ...
    if (cachedPdfs.length === 0 && cachedVideos.length === 0) {
      issues.push({
        id: "empty-content-db",
        title: "स्टडी कंटेंट डेटाबेस रिक्त (Empty) है",
        description: "कोई भी PDF या वीडियो नहीं मिला। ऐप पूरी तरह खाली दिखेगा।",
        severity: "critical",
        category: "integrity",
        suggestedAction: "डिफ़ॉल्ट BSEB Class 10th संपूर्ण स्टडी किट रीस्टोर करें।",
        autoFixPayload: {
          actionType: "RENEW_RESOURCES",
          payload: {}
        }
    ...
    return {
      ...
      issues,
      metrics: {
        totalPdfs: cachedPdfs.length,
        totalVideos: cachedVideos.length,
        totalMusic: cachedMusic.length,
        totalAnnouncements: cachedAnnouncements.length,
        brokenLinksCount: pdfsWithMissingUrl.length + videosWithMissingId.length,
        flagsStatus: { ...cachedFeatureFlags },
        brandingConfigured: Boolean(cachedBranding.appName)
      }
    };
  },
